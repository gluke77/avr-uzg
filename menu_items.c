...
void menu_freq(void)
{
	sprintf(lcd_line1, "◊¿—“Œ“¿=%-12ld  ", g_dds_freq);
...
void menu_amp(void)
{
	...
	sprintf(lcd_line1, "¿ÃœÀ»“”ƒ¿:%-10d", amp);
...
void menu_temp(void)
{
	sprintf(lcd_line1, "“≈Ãœ≈–¿“”–¿:%.1fC               ", temp_value());
...
{
	sprintf(lcd_line1, "“Œ =%.2fA            ", bias_pwm_to_current(g_bias_pwm));
...
{
	sprintf(lcd_line1, "ÃŒŸÕŒ—“‹=%2d%%            ", g_power_pwm + 1);
...
{
	sprintf(lcd_line1, "Õ¿◊¿À‹Õ€… “Œ =%.2fA     ", bias_pwm_to_current(g_bias_pwm_base));
...
{
	sprintf(lcd_line1, "ƒŒ¡¿¬ ¿ “Œ ¿=%.2fA       ", bias_pwm_to_current(g_bias_pwm_shift));
...
{
	sprintf(lcd_line1, "Õ¿◊. ÃŒŸÕŒ—“‹=%2d%%   ", g_power_pwm_base + 1);
...
{
	sprintf(lcd_line1, "ƒŒ¡¿¬ ¿ ÃŒŸÕŒ—“»=%2d%%   ", g_power_pwm_shift);
...
void menu_max_power_pwm(void)
{
	...
	sprintf(lcd_line1, "Ã¿ —. ÃŒŸÕŒ—“‹=%d%%  ", g_max_power_pwm + 1);
...
void menu_min_power_pwm(void)
{
	...
	sprintf(lcd_line1, "Ã»Õ. ÃŒŸÕŒ—“‹=%d%%    ", g_min_power_pwm + 1);
		...
		if (g_dds_freq < g_freq_upper)
		{
			...
			else
			{
				...
				{		
					...
					sprintf(lcd_line1, "œŒ»—  F:%-5ld C:%-4.2f", g_dds_freq, adc_to_current(current));
...
void menu_freq_lower(void)
{
	...
	sprintf(lcd_line1, "Ã»Õ»Ã¿À‹Õ¿ﬂ F=%-6ld", g_freq_lower);
...
void menu_freq_upper(void)
{
	...
	sprintf(lcd_line1, "Ã¿ —»Ã¿À‹Õ¿ﬂ F=%-5ld", g_freq_upper);

	menu_common();
}

char	pfc_mode_str[PFC_COUNT][5] = {"", "¬€ À", "¬ À", "¿¬“Œ"};
...
void menu_pfc_mode(void)
{
	...
	sprintf(lcd_line1, " Ã:%-17s", pfc_mode_str[g_pfc_mode]);
...
char	keep_mode_str[KEEP_COUNT][5] = {"","¬€ À", "“Œ ", "¿Ãœ."};
...
void menu_keep_mode(void)
{
	...
	sprintf(lcd_line1, "¿¬“ŒœŒƒ—“–Œ… ¿:%-5s", keep_mode_str[g_keep_mode]);
...
void menu_autosearch_mode(void)
{
	...
	sprintf(lcd_line1, "œŒ»—  œ–» œ”— ≈:%-4s",	(AUTOSEARCH_ON == g_autosearch_mode)?"¬ À":"¬€ À");
...
void menu_fault_interrupts(void)
{
	...
	sprintf(lcd_line1, "«¿Ÿ»“¿:%-13s",	(FAULT_INTERRUPTS_ON == g_fault_interrupts_mode)?"¬ À":"¬€ À");
...
void menu_keep_step(void)
{
	...
	sprintf(lcd_line1, "ÿ¿√ ¿¬“ŒœŒƒ—“–.=%-4d", g_keep_freq_step);
...
void menu_keep_delta(void)
{
	...
	sprintf(lcd_line1, "ƒ»¿œ. ¿¬“ŒœŒƒ—“–.=%-2d", g_keep_freq_max_delta);
...
{
	sprintf(lcd_line1, "»Õ“≈√–. «¿ƒ≈–∆ ¿=%-3d", g_int_timeout);
...
{
	sprintf(lcd_line1, "¿ƒ–≈—=%-14d", g_modbus_id);
...
void menu_baudrate(void)
{
	...
	sprintf(lcd_line1, "— Œ–Œ—“‹=%-11ld", g_baudrate);
...
void menu_temp_alarm(void)
{
	...
	sprintf(lcd_line1, "“≈Ãœ.  –»“»◊.=%-d—     ", g_temp_alarm);
...
void menu_temp_stop(void)
{
	...
	sprintf(lcd_line1, "“≈Ãœ. Œ—“¿ÕŒ¬¿=%-d—    ", g_temp_stop);
	
	menu_common();
}

char	stop_mode_str[5][15] = {"œ”À‹“", "485", "œ≈–≈√–≈¬", "œ≈–≈√–. œŒ F", "œ≈–≈√–. œŒ “Œ ”"};

void menu_stop_mode(void)
{
	sprintf(lcd_line1, "—“Œœ:%-15s", stop_mode_str[(uint8_t)g_stop_mode]);
	menu_common();
}

void menu_version(void)
{
	sprintf(lcd_line1, "¬≈–—»ﬂ œŒ:%-10s", FW_VERSION);
	menu_common();
}
